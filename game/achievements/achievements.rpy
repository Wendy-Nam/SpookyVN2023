...
screen achievement_gallery():
    ...
    ## use the following:
    # textbutton _("Return") action Return() align (0.9, 0.85)
    # viewport:
    ...
    $ achive_percent = ExtraAnimatedValue(
                    value=Achievement.num_earned(), 
                    range=Achievement.num_total(), 
                    range_delay=3.5,
                    warper="easein_bounce")
    # add Bar(value=achive_percent, xminimum=int(renpy.config.screen_width * 0.8), yminimum=80)
    # add Text(achive_percent.text('{.percent:.01%}', size = 30,
    #         color = "#DDE",
    #         outlines = [(abs(1), "#222")],
    #         bold = True,
    #         xcenter = 0.5,
    ...
    hbox at percent_bar:
        ...
        bar:
            ...
            xmaximum 700
            yalign 0.7
        # area (0,0, int(renpy.config.screen_width * 0.8), 80)
        # bar:
        #     value achive_percent
        #     xminimum int(renpy.config.screen_width * 0.8)
        #     yminimum 80
        # add achive_percent.text('{.percent:.01%}',
        #     size = 30,
        #     color = "#DDE",
        #     outlines = [(abs(1), "#222")],
        #     bold = True,
